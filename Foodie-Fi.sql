...
WHERE s.customer_id IN (1,2,11,13,15,16,18,19);
  ...
  -- Que 2 What is the monthly distribution of trial plan start_date values for our dataset - use the start of the month as the group by value
with cte1 as 
(select *, month(start_date) as mon 
from subscriptions)
select  c.mon , count(c.mon) as mon_count 
from plans as p
inner join
cte1 as c
...
-- Que 3 What plan start_date values occur after the year 2020 for our dataset? Show the breakdown by count of events for each plan_name
select p.plan_name, count(s.yr)
from (select *, year(start_date) as yr from subscriptions) as s  
inner join 
plans as p
...
-- Que 5 How many customers have churned straight after their initial free trial - what percentage is this rounded to the nearest whole number?
select count(*)  as customer_churn,
round(count(*) / (select count(*) from subscriptions where plan_id = 4) * 100,1) as customer_churn_per
from (select *, rank() over(partition by customer_id order by start_date) as rnk
...
-- Que 6 What is the number and percentage of customer plans after their initial free trial?
 with t1 as (select s.*, p.plan_name, rank() over (partition by customer_id order by start_date) as rnk
from subscriptions as s
inner join 
plans as p
on s.plan_id  = p.plan_id)
select plan_id, plan_name, count(customer_id) as num_customers, 
round(count(customer_id) / ( select count(customer_id) from t1 where rnk =2) *100, 1) as customer_per
from t1 
where rnk = 2
...
select plan_name, count(distinct customer_id) as customers,
	concat(round(100*count(distinct customer_id) / (select count(distinct customer_id) from subscriptions),1), '%')as percentage
from cte c
...
from plans as p
inner join 
(select *, year(start_date) as yr from subscriptions) as s
on p.plan_id = s.plan_id
where yr = 2020 and plan_name = 'pro annual';

-- Que 9 How many days on average does it take for a customer to an annual plan from the day they join Foodie-Fi?

with cte1 as (
select t2.customer_id , t3.t3_customer_id, t2.plan_id, t3.t3_plan_id, t2.start_date, t3.annual_plan_date 
from (
select * from (
	select *, rank() over (partition by customer_id order by start_date) as rnk from subscriptions
    ) as t1
where rnk = 1) as t2
inner join 
(select customer_id as t3_customer_id, plan_id as t3_plan_id, start_date as annual_plan_date from subscriptions where plan_id = 3) as t3
where t2.customer_id = t3.t3_customer_id)

select round(avg(datediff( annual_plan_date, start_date)),0) as avg_days from cte1;
